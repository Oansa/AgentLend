...
function getAddress(value) {
  ...
  const hash5 = keccak2562(concat(fromString(accountDomain), id));
...
function getSignPayload(value) {
  ...
  return keccak2562(concat(fromString(signatureDomain), from(payload), account, genesisConfigId));
}
function toId(config) {
  assert4(config);
  const id = keccak2562(concat(fromString(configDomain), padLeft2(config.salt ?? zeroSalt, 32), fromNumber(config.threshold, { size: 4 }), fromNumber(config.owners.length, { size: 4 }), ...config.owners.flatMap((owner) => [
    owner.owner,
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "multisig") {
    const multisig = envelope;
    return concat(serializedMultisigType, fromHex3([
      multisig.account,
...
function hash(authorization, options = {}) {
  ...
  return keccak2562(concat("0x05", fromHex3(toTuple3(presign ? {
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload4(from15(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload4(from15(envelope)),
...
async function resolveNode() {
  ...
  const { wasmBase64 } = await import("./mine.wasm-74YBOYBT.js");
...
function getRegistrationHash(value) {
  return keccak2562(concat(resolveAddress3(value.address), toFixedHex2(value.salt, 32)));
}
...
function validateSalt(value) {
  try {
    return hasProofOfWork(keccak2562(concat(resolveAddress3(value.address), toFixedHex2(value.salt, 32)), { as: "Bytes" }));
  } catch {
    return false;
  }
}
...
var fieldsSize = 29;
var version = 0;
function from17(authentication, options = {}) {
  ...
  async verifyHash(client, parameters2) {
    ...
    if (envelope) {
      if ((envelope == null ? void 0 : envelope.type) === "keychain" && mode2 === "allowAccessKey") {
        const innerPayload = envelope.version === "v2" ? keccak256(concat("0x04", hash5, address2)) : hash5;
...
//# sourceMappingURL=chunk-5O7DBB25.js.map
